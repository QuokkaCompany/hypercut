...
type Props = { onEmphasize: (cueId: string) => void; glossary: string; onGlossaryChange: (value: string) => void; media: Media; trackIndex: number; transcript: Transcript | null; captionStyle: Style; onStyleChange: (style: Style) => void; onRenderPreview: () => void; kept: { start: number; end: number }[]; busy: boolean; job: Job | null; canUndo: boolean; canRedo: boolean; onUndo: () => void; onRedo: () => void; onChange: (value: Transcript) => void; onTranscribe: (settings: TranscriptionSettings) => void; onExport: () => void; onExportText: (source: boolean) => void; onCreateClip: (range: { start: number; end: number }) => void; onCancel: () => void; onClose: () => void };
export function CaptionEditor({ onEmphasize, glossary, onGlossaryChange, media, trackIndex, transcript, captionStyle, onStyleChange, onRenderPreview, kept, busy, job, canUndo, canRedo, onUndo, onRedo, onChange, onTranscribe, onExport, onExportText, onCreateClip, onCancel, onClose }: Props) {
  ...
  return <div className="modal-backdrop caption-backdrop"><section className="modal caption-modal" role="dialog" aria-modal="true" aria-label="전사와 자막 편집">
    ...
    <div className="caption-workspace"><div className="caption-source">
      ...
      {cue && <button className="button secondary" disabled={busy || mismatch || draftDirty} onClick={() => { player.current?.pause(); onEmphasize(cue.id); }}>이 문장 강조하기</button>}
