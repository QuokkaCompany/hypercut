...
export function AIAssistant({ settings = DEFAULT_SETTINGS, contextId, disabled, onApply, onClose, captionTask, effectsTask }: Props) {
  ...
  return <div className="modal-backdrop"><section className={`modal ai-modal ${correcting || arranging ? 'correction-modal' : ''}`} role="dialog" aria-modal="true" aria-label={translating ? 'AI 자막 번역' : arranging ? 'AI 효과음 제안' : correcting ? 'AI 자막 교정' : 'AI 편집 도우미'}>
    ...
    <label className="ai-field">사용할 AI<select aria-label="사용할 AI" value={provider} onChange={e => void changeProvider(e.target.value as Provider)} disabled={working}><option value="manual">ChatGPT · Claude 채팅 — 복사해서 사용</option><option value="mcp">외부 AI — MCP 연결 (실험)</option><option value="claude_cli">Claude Code — 기존 구독 로그인</option><option value="ollama">Ollama — 로컬 모델</option><option value="openai">OpenAI API — 별도 API 키</option><option value="anthropic">Claude API — 별도 API 키</option></select></label>
