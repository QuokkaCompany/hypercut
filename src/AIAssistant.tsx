...
import type { Settings, CorrectionRequest, CorrectionProposal } from './types';
...
type Props = { contextId: string; disabled: boolean; onClose: () => void } & ({ settings: Settings; onApply: (settings: Settings) => void; captionTask?: never } | { settings?: never; onApply?: never; captionTask: { cues: CorrectionRequest['cues']; onApply: (request: CorrectionRequest, proposal: CorrectionProposal, ids: string[]) => void } });
export function AIAssistant({ settings = DEFAULT_SETTINGS, contextId, disabled, onApply, onClose, captionTask }: Props) {
  const correcting = !!captionTask, route = correcting ? '/ai/correction' : '/ai/proposal';
  ...
  const [instruction, setInstruction] = useState(correcting ? '맞춤법과 띄어쓰기 오타를 고쳐 주세요. 숫자·단위·고유명사·부정 의미는 유지해 주세요.' : '작은 목소리와 짧은 쉼은 살리고, 긴 무음만 줄이고 싶어요.');
  ...
  const context = JSON.stringify([contextId, correcting ? captionTask.cues : settings]);
  ...
  useEffect(() => { if (previousContext.current !== context) { previousContext.current = context; revision.current++; requestController.current?.abort(); setProposal(null); setCorrection(null); setCorrectionRequest(null); setPrompt(''); setWorking(false); void cancelRemote().catch(() => {}); } }, [context, route]);
  function clearProposal() { revision.current++; setProposal(null); setCorrection(null); setError(''); setNotice(''); }
  function invalidateRequest() { clearProposal(); setPrompt(''); setCorrectionRequest(null); }
  ...
  async function ask() {
    setError(''); setNotice(''); setProposal(null); setCorrection(null); setWorking(true); setVerified(false); setExecution(null); const current = ++revision.current;
    try {
      const controller = new AbortController(); requestController.current = controller;
      const snapshot = correcting ? prepareCorrection() : null;
      ...
      if (mounted.current && current === revision.current) {
        if (snapshot) setCorrection(validateCorrectionProposal(result, snapshot)); else setProposal(validateProposal(result)); setVerified(true);
  ...
  return <div className="modal-backdrop"><section className={`modal ai-modal ${correcting ? 'correction-modal' : ''}`} role="dialog" aria-modal="true" aria-label={correcting ? 'AI 자막 교정' : 'AI 편집 도우미'}>
    <div className="panel-heading"><h2><Sparkles size={19} />{correcting ? 'AI 자막 교정' : 'AI 편집 도우미'}</h2><button className="icon-button" aria-label="AI 창 닫기" onClick={onClose}><X size={18} /></button></div>
    <p className="ai-intro">{correcting ? `선택한 자막 ${captionTask.cues.length}개 · ${captionTask.cues.reduce((sum, cue) => sum + cue.text.length, 0)}자를 교정하고 필요한 제안만 적용합니다.` : '원하는 편집을 말하면 무음 설정을 제안받을 수 있어요.'}</p>
    ...
    {provider === 'manual' ? <>
      ...
      <button className="button secondary" disabled={!response.trim()} onClick={() => { try { if (correcting) { if (!correctionRequest) throw new Error('현재 자막의 요청을 먼저 복사해 주세요.'); setCorrection(validateCorrectionProposal(response, correctionRequest)); } else setProposal(validateProposal(response)); setError(''); } catch (e) { setError((e as Error).message); } }}><Check size={15} />응답 확인</button>
