import { translationPrompt, validateTranslationRequest, validateTranslationProposal } from '../shared/caption-translation.mjs';
import { languageLabel } from '../shared/languages.mjs';
...
import { request, cloudMode } from './api';
...
import { MCPShare } from './MCPShare';

type Provider = 'manual' | 'mcp' | 'ollama' | 'openai' | 'anthropic' | 'claude_cli';
...
type Props = { contextId: string; disabled: boolean; onClose: () => void } & ({ settings: Settings; onApply: (settings: Settings) => void; captionTask?: never; effectsTask?: never } | { settings?: never; onApply?: never; captionTask: { targetLanguage?: CorrectionRequest['targetLanguage']; glossary: string; cues: CorrectionRequest['cues']; onApply: (request: CorrectionRequest, proposal: CorrectionProposal, ids: string[]) => void }; effectsTask?: never } | { settings?: never; onApply?: never; captionTask?: never; effectsTask: { context: EffectAIContext; onApply: (request: EffectAIRequest, proposal: EffectAIProposal, ids: string[]) => void } });
export function AIAssistant({ settings = DEFAULT_SETTINGS, contextId, disabled, onApply, onClose, captionTask, effectsTask }: Props) {
  const translating = !!captionTask?.targetLanguage;
  const captionRequest = translating ? validateTranslationRequest : validateCorrectionRequest, captionProposal = translating ? validateTranslationProposal : validateCorrectionProposal, captionPrompt = translating ? translationPrompt : correctionPrompt;
  const correcting = !!captionTask, arranging = !!effectsTask, route = arranging ? '/ai/effects' : translating ? '/ai/translation' : correcting ? '/ai/correction' : '/ai/proposal';
  ...
  const [instruction, setInstruction] = useState(translating ? '각 자막을 자연스럽게 번역해 주세요. 사실·숫자·단위·고유명사·부정 의미를 유지해 주세요.' : arranging ? '설명과 선택한 자막에 맞춰 효과음을 짧고 자연스럽게 배치해 주세요. 기존 클립을 수정할 때는 말소리가 잘 들리도록 음량을 조절해 주세요.' : correcting ? '맞춤법과 띄어쓰기 오타를 고쳐 주세요. 숫자·단위·고유명사·부정 의미는 유지해 주세요.' : '작은 목소리와 짧은 쉼은 살리고, 긴 무음만 줄이고 싶어요.');
  ...
  const context = JSON.stringify([contextId, arranging ? effectsTask.context : correcting ? [captionTask.cues, captionTask.glossary, captionTask.targetLanguage] : settings]);
  ...
  function prepareCorrection() { const value = captionRequest({ targetLanguage: captionTask?.targetLanguage, requestId: crypto.randomUUID(), instruction, glossary, cues: captionTask?.cues }) as CorrectionRequest; setCorrectionRequest(value); return value; }
  ...
  async function ask() {
    ...
    try {
      ...
      if (mounted.current && current === revision.current) {
        if (snapshot && arranging) setEffectProposal(validateEffectProposal(result, snapshot) as EffectAIProposal); else if (snapshot) setCorrection(captionProposal(result, snapshot)); else setProposal(validateProposal(result)); setVerified(true);
  ...
  async function copyPrompt() {
    ...
    let text;
    try {
      text = arranging ? effectPrompt(prepareEffects()) : correcting ? captionPrompt(prepareCorrection()) : proposalPrompt(instruction, settings); setPrompt(text); setError('');
  ...
  return <div className="modal-backdrop"><section className={`modal ai-modal ${correcting || arranging ? 'correction-modal' : ''}`} role="dialog" aria-modal="true" aria-label={translating ? 'AI 자막 번역' : arranging ? 'AI 효과음 제안' : correcting ? 'AI 자막 교정' : 'AI 편집 도우미'}>
    <div className="panel-heading"><h2><Sparkles size={19} />{translating ? 'AI 자막 번역' : arranging ? 'AI 효과음 제안' : correcting ? 'AI 자막 교정' : 'AI 편집 도우미'}</h2><button className="icon-button" aria-label="AI 창 닫기" onClick={onClose}><X size={18} /></button></div>
    <p className="ai-intro">{translating ? `${languageLabel(captionTask.targetLanguage)} 번역 · 자막 ${captionTask.cues.length}개. 원문은 보존하고 선택한 번역을 별도로 저장합니다.` : arranging ? `선택한 음원 ${effectsTask.context.assets.length}개 · 기존 클립 ${effectsTask.context.clips.length}개 · 참고 자막 ${effectsTask.context.cues.length}개로 효과음 편집을 제안합니다.` : correcting ? `선택한 자막 ${captionTask.cues.length}개 · ${captionTask.cues.reduce((sum, cue) => sum + cue.text.length, 0)}자를 교정하고 필요한 제안만 적용합니다.` : '원하는 편집을 말하면 무음 설정을 제안받을 수 있어요.'}</p>
    <label className="ai-field">사용할 AI<select aria-label="사용할 AI" value={provider} onChange={e => void changeProvider(e.target.value as Provider)} disabled={working}><option value="manual">ChatGPT · Claude 채팅 — 복사해서 사용</option>{!cloudMode && <><option value="mcp">외부 AI — MCP 연결 (실험)</option><option value="claude_cli">Claude Code — 기존 구독 로그인</option><option value="ollama">Ollama — 로컬 모델</option></>}<option value="openai">OpenAI API — 별도 API 키</option><option value="anthropic">Claude API — 별도 API 키</option></select></label>
    {provider !== 'manual' && provider !== 'mcp' && <div className="ai-connection">
    ...
    <div className="modal-note">{provider === 'mcp' ? '선택한 요청을 공유하면 외부 AI가 편집 제안을 앱에 전달합니다. 적용 전 비교와 선택은 이 앱에서 진행합니다.' : cloud ? 'API 사용료는 ChatGPT·Claude 채팅 구독과 별도일 수 있어요.' : provider === 'claude_cli' ? '설치된 Claude Code의 기존 구독 로그인으로 요청합니다. 제안 요청 시 해당 계정의 사용량이 적용됩니다. 파일·셸·외부 도구는 사용할 수 없습니다.' : provider === 'ollama' ? '이 컴퓨터에서 실행하는 Ollama 모델을 사용합니다. 서버가 꺼져 있어도 무음 편집은 계속할 수 있어요.' : '사용 중인 ChatGPT·Claude 채팅에 요청을 붙여넣고, 받은 JSON 응답을 가져오세요. 자동 로그인 연결은 아닙니다.'} {arranging ? '선택한 음원의 별칭·설명·길이, 클립과 참고 자막의 원본 시각, 영상 길이·유지 구간을 전달합니다.' : correcting ? '선택한 자막 문구·용어·지시와 번역할 언어만 전달합니다. 시각은 전달하거나 바꾸지 않습니다.' : '요청 문장과 네 가지 설정만 전달합니다.'} 영상·음성·파일 이름은 보내지 않습니다.</div>
    {correcting && <><details className="ai-details correction-input"><summary>AI에 보낼 자막 {captionTask.cues.length}개 보기</summary>{captionTask.cues.map((cue, i) => <p key={cue.id}>{i + 1}. {cue.text}</p>)}</details><label className="ai-field">이번 교정에 참고할 용어<textarea aria-label="교정 참고 용어" value={glossary} maxLength={GLOSSARY_MAX_LENGTH} rows={2} disabled={working} placeholder="예: 캡컶 → 캡컷, Whisper → 위스퍼" onChange={e => { setGlossary(e.target.value); invalidateRequest(); }} /></label><p className="field-hint">프로젝트 용어를 기본으로 채웠습니다. 여기서 바꾸거나 비우면 이번 요청에만 반영됩니다. {translating ? '번역의 숫자·단위·의미를 원문과 비교해 주세요.' : '숫자가 바뀌는 제안은 거부합니다.'}</p><button className="text-button" disabled={working || glossary === captionTask.glossary} onClick={() => { setGlossary(captionTask.glossary); invalidateRequest(); }}>프로젝트 용어로 되돌리기</button></>}
    <label className="ai-field">{translating ? '어떻게 번역할까요?' : correcting ? '어떻게 교정할까요?' : '어떻게 편집할까요?'}<textarea value={instruction} maxLength={2000} rows={3} disabled={working} onChange={e => { setInstruction(e.target.value); invalidateRequest(); }} /></label>
    {provider === 'mcp' ? <MCPShare contextKey={context} instruction={instruction} glossary={glossary} settings={settings} disabled={disabled} onApply={onApply} captionTask={captionTask} effectsTask={effectsTask} onClose={onClose} /> : provider === 'manual' ? <>
    ...
    {correction && correctionRequest && captionTask && <CorrectionReview translation={translating} key={JSON.stringify(correction)} proposal={correction} disabled={disabled || working} onApply={ids => { try { captionTask.onApply(correctionRequest, correction, ids); onClose(); } catch (e) { setError((e as Error).message); } }} />}
  </section></div>;
}
