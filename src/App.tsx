import { VisualAccents } from './VisualAccents';
...
import type { CloudEditor } from './Cloud';
...
export default function App({ cloud }: { cloud?: CloudEditor } = {}) {
  ...
  const [accentsOpen, setAccentsOpen] = useState(false), [accentCue, setAccentCue] = useState<string | undefined>();
  const accentsHistory = useHistory<VisualAccent[]>([]);
  const visualAccents = accentsHistory.value;
  function changeAccents(next: VisualAccent[]) { accentsHistory.edit(next); setUnsaved(true); }
  function openAccents(cueId?: string) { video.current?.pause(); setAccentCue(cueId); setAccentsOpen(true); }
  const [captionsOpen, setCaptionsOpen] = useState(false);
  const [effectsOpen, setEffectsOpen] = useState(false), [connectedEffects, setConnectedEffects] = useState<Set<string>>(new Set());
  const effectsHistory = useHistory<Effects>({ assets: [], clips: [] });
  const effects = effectsHistory.value;
  function changeEffects(next: Effects) { effectsHistory.edit(next); setUnsaved(true); }
  function openEffects() { video.current?.pause(); setEffectsOpen(true); }
  function setEffectImportBusy(value: boolean) {
    // Keep an earlier project read from replacing the target of this import.
    if (value) operation.current++;
    setBusy(value ? 'effect-import' : null);
  }
  const captionHistory = useHistory<{ transcript: Transcript | null; style: CaptionStyle; glossary: string }>({ transcript: null, style: { ...DEFAULT_CAPTION_STYLE } as CaptionStyle, glossary: '' });
  const { transcript, style: captionStyle, glossary } = captionHistory.value;
  function changeGlossary(next: string) { captionHistory.edit({ ...captionHistory.value, glossary: next }); setUnsaved(true); }
  ...
  useEffect(() => { if (ready && cloud?.initial && !cloudInitialized.current) { cloudInitialized.current = true; pendingProject.current = cloud.initial.project.data; installMedia(cloud.initial.media); setConnectedEffects(new Set(cloud.initial.project.data.effects.assets.map((x: { id: string }) => x.id))); } }, [ready, cloud]);
  ...
  useEffect(() => { setRendered(null); setOutput(null); setRangePreview(null); setMode(old => old === 'rendered' ? 'edited' : old); }, [cuts, trackIndex, transcript, captionStyle, effects, visualAccents]);
  ...
  useEffect(() => {
    const onKey = (event: KeyboardEvent) => {
      ...
      if (dialog || aiOpen || accentsOpen || captionsOpen || effectsOpen || restoreOpen || clipRange || previewRangeOpen || rangePreview || target.isContentEditable || ['INPUT', 'TEXTAREA', 'SELECT'].includes(target.tagName)) return;
  ...
  function installMedia(next: Media) {
    ...
    accentsHistory.load((project?.visualAccents || []) as VisualAccent[]);
    ...
    setClipRange(null); setPreviewRangeOpen(false); setRangePreview(null);
  ...
  async function run(type: 'analyze' | 'preview' | 'export' | 'restore' | 'transcribe' | 'captions' | 'transcript', range?: { start: number; end: number }, transcription?: TranscriptionSettings, textMode?: 'source' | 'edited') {
    ...
    try {
      const cloudContext = cloud ? await cloud.save(makeProject(media, settings, trackIndex, cuts, speechProtection, transcript, captionStyle, effects, glossary, visualAccents), media) : {};
      if (active.cancelled) return;
      const started = await request<Job>('/jobs', { ...cloudContext, type, mediaId: media.id, settings, speechProtection, trackIndex, cuts, requestId: active.id, range, transcription, transcript, captionStyle, effects, visualAccents, textMode }, undefined, active.controller.signal);
      ...
      if (type === 'transcribe') { changeTranscript(completed.result as Transcript); setNotice('전사가 완료됐습니다. 문구와 시각을 확인해 주세요.'); }
      else if (type === 'captions' || type === 'transcript') {
        const result = completed.result as Output;
        if (window.hypercut) { if (await window.hypercut.saveExport(result.id)) setNotice(type === 'transcript' ? '대본 TXT를 저장했습니다.' : '편집한 자막을 저장했습니다.'); }
        else { const anchor = document.createElement('a'); anchor.href = outputURL(result.id, true); anchor.download = result.name; anchor.click(); setNotice(type === 'transcript' ? '대본 TXT 다운로드를 시작했습니다.' : '편집한 SRT 자막 다운로드를 시작했습니다.'); }
      }
  ...
  async function saveProject() {
    ...
    let data;
    try { data = makeProject(media, settings, trackIndex, cuts, speechProtection, transcript, captionStyle, effects, glossary, visualAccents); }
    catch (e) { setError((e as Error).message); return; }
    if (cloud) {
      const epoch = projectEpoch.current, revision = editRevision.current;
      saveActive.current = true; setSaving(true);
      try { await cloud.save(data, media); if (projectEpoch.current === epoch && editRevision.current === revision) setUnsaved(false); setNotice('Project saved on the server.'); }
  ...
  return <div className={`app-shell${cloud ? ' cloud-editor' : ''}`}>
    ...
    <header className="app-header">
      ...
      <div className="header-actions">{cloud && media && <button className="button ghost" disabled={!!busy || saving} onClick={() => { const data = makeProject(media, settings, trackIndex, cuts, speechProtection, transcript, captionStyle, effects, glossary, visualAccents); const url = URL.createObjectURL(new Blob([JSON.stringify(data, null, 2)], { type: 'application/json' })); const anchor = document.createElement('a'); anchor.href = url; anchor.download = `${media.name}.hypercut.json`; anchor.click(); setTimeout(() => URL.revokeObjectURL(url), 1000); }}>Download project</button>}{cloud && <button className="button secondary" onClick={() => { if ((!unsaved && !saving) || confirm('Return to the workspace? Unsaved edits will be lost. Submitted jobs continue on the server.')) cloud.exit(); }}>Workspace</button>}<span className={`local-status ${engineError ? 'error-status' : ''}`}><i />{engineError ? '엔진 확인 필요' : (cloud ? '서버에서 처리' : '내 컴퓨터에서 처리')}</span><button className="button ghost compact" aria-label="프로젝트 저장" onClick={saveProject} disabled={!media || !!busy || saving}><Save size={15} />{saving ? '저장 중…' : '프로젝트 저장'}</button><button className="button secondary" disabled={!media || !!busy || !media.audioTracks.length} onClick={() => { video.current?.pause(); setClipRange({ start: Math.min(time, Math.max(0, media!.duration - .001)), end: media!.duration }); }}>클립 만들기</button><button className="button primary" onClick={() => run('export')} disabled={!media || !kept.length || !!busy || !media.audioTracks.length}><ArrowUpFromLine size={16} />내보내기</button></div>
    </header>
    <div className="editor-shell">
      <nav className="tool-rail" aria-label="앱 메뉴"><button className="rail-button active" title="영상 편집" aria-label="영상 편집"><Scissors size={21} /><span>편집</span></button><button className="rail-button" title="AI 편집 도우미" aria-label="AI 편집 도우미" onClick={() => setAIOpen(true)} disabled={!ready || !!busy}><Sparkles size={20} /><span>AI</span></button><button className="rail-button" aria-label="전사와 자막" title="전사와 자막" disabled={!media || !!busy || !media.audioTracks.length} onClick={openCaptions}><Captions size={20} /><span>자막</span></button><button className="rail-button" aria-label="효과음 편집" title="효과음 편집" disabled={!media || !!busy || !media.audioTracks.length} onClick={openEffects}><Music2 size={20} /><span>효과음</span></button><button className="rail-button" aria-label="문장 강조와 줌" title="문장 강조와 줌" disabled={!media || !!busy} onClick={() => openAccents()}><Sparkles size={21} /><span>문장 강조</span></button><div className="rail-bottom"><button className="rail-button" onClick={() => setDialog('engine')} title="로컬 엔진" aria-label="로컬 엔진"><Settings2 size={19} /></button><button className="rail-button" onClick={() => setDialog('help')} title="사용 방법" aria-label="사용 방법"><CircleHelp size={19} /></button><span className="avatar">H</span></div></nav>
      <aside className="media-panel"><div className="panel-heading"><h2>프로젝트</h2><button className="icon-button" onClick={chooseVideo} disabled={!!busy || !ready} title="영상 추가" aria-label="영상 추가"><Plus size={17} /></button></div>
        ...
        <div className="privacy-note"><ShieldCheck size={15} /><span>{cloud ? '영상은 연결된 서버에 업로드됩니다.' : '영상은 이 컴퓨터에 머뭅니다.'}</span></div>
      </aside>
      <main className="main-editor">
        ...
        <div className={`preview-stage ${dragging ? 'dragging' : ''}`} onDragOver={e => { e.preventDefault(); if (!busy) setDragging(true); }} onDragLeave={() => setDragging(false)} onDrop={e => { e.preventDefault(); setDragging(false); const file = e.dataTransfer.files[0]; if (file) void importFile(file); }}>
          {media ? <><video ref={video} src={videoSrc} playsInline preload="auto" onPlay={() => setPlaying(true)} onPause={() => setPlaying(false)} onEnded={() => setPlaying(false)} onTimeUpdate={() => { const t = video.current?.currentTime ?? 0; setTime(mode === 'rendered' ? editedToSource(t, kept) : t); }} onError={() => setError('미리보기 재생에 실패했습니다. 다른 H.264 영상을 선택하거나 엔진 상태를 확인해 주세요.')} /><div className="preview-badge"><i />{mode === 'original' ? '원본 영상' : mode === 'rendered' ? '렌더링된 편집본' : '컷 미리보기 · 강조 효과 미포함'}</div>{media.name === 'HyperCut 검증 샘플.mp4' && <span className="synthetic-badge">합성 검증 샘플 · 실제 음성 아님</span>}</> : <div className="empty-stage"><div className="empty-art"><span /><AudioLines size={48} strokeWidth={1.2} /><div className="art-cut"><Scissors size={16} /></div></div><span className="eyebrow">LESS EDITING. MORE CREATING.</span><h1>말은 그대로,<br /><em>쉼만 가볍게.</em></h1><p>영상을 넣고 무음 구간을 한 번에 정리하세요.<br />반복되는 컷편집은 HyperCut에 맡겨두세요.</p><button className="button primary large" onClick={chooseVideo} disabled={!!busy || !ready || !!engineError}><Plus size={18} />영상 불러오기</button><span className="drop-hint">또는 이곳에 끌어놓기 · MP4, MOV / H.264</span><button className="text-button demo-button" onClick={loadDemo} hidden={!!cloud} disabled={!!busy || !ready || !!engineError}><Play size={12} />16초 샘플로 먼저 체험하기<ArrowRight size={13} /></button></div>}
          {busy && !effectsOpen && <div className="job-overlay"><div className="job-card"><LoaderCircle size={26} className="spin" /><strong>{busy === 'import' ? (importProgress >= 1 ? '영상 정보를 확인하고 있어요' : '영상을 준비하고 있어요') : job?.stage || '작업을 준비하고 있어요'}</strong><div className="progress-track"><i style={{ width: `${busy === 'import' ? Math.max(5, importProgress * 100) : Math.max(3, (job?.progress || 0) * 100)}%` }} /></div><div className="job-foot"><span>{busy === 'import' ? (cloud ? '서버 업로드 · 취소하면 일시 정지' : '로컬 파일 처리 중') : `${Math.round((job?.progress || 0) * 100)}%`}</span>{(busy !== 'import' || importController.current) && <button className="text-button" onClick={cancel}>작업 취소</button>}</div></div></div>}
        </div>
      ...
      <aside className="settings-panel"><div className="panel-heading"><h2><Scissors size={16} />무음 자동 편집</h2><span className="small-tag">{cloud ? 'CLOUD' : 'LOCAL'}</span></div><div className="settings-content"><p className="panel-description">듣고 싶은 말만 남기세요.<br />편집 기준은 직접 조절할 수 있어요.</p>
        ...
        <p className="analysis-note">{cloud ? 'AI 계정 없이 서버에서 분석합니다.' : 'AI 계정 없이, 내 컴퓨터에서 분석합니다.'}</p>
        ...
        {output && <div className="export-ready"><div><Check size={17} /><strong>{output.sourceRange ? "클립 내보내기 완료" : "내보내기 완료"}</strong></div><p>{output.sourceRange && <>원본 {formatTime(output.sourceRange.start, true)}–{formatTime(output.sourceRange.end, true)} · </>}{formatTime(output.duration)} · {formatSize(output.size)} · 파일 검증 완료{output.captionStyle?.enabled ? ` · 자막 ${output.burnedCaptions}개 포함` : ' · 자막 없음'}{output.audioMix?.mixedClips ? ` · 효과음 ${output.audioMix.mixedClips}개 · 최종 피크 ${(output.audioMix.encodedPeakDb ?? output.audioMix.peakDb)?.toFixed(1) ?? '−∞'} dBFS` : ''}</p><button className="button primary" disabled={!!busy} onClick={downloadOutput}><Download size={15} />{output.sourceRange ? "클립 MP4 저장" : "편집한 MP4 저장"}</button></div>}
      </div></aside>
    </div>
    <footer className="status-bar"><span><HardDrive size={12} />{window.hypercut ? '데스크톱 앱' : '브라우저 앱'}<i />{cloud ? '클라우드 편집' : '로컬 편집'}</span><span>{media ? `${media.width} × ${media.height} · ${media.fps.toFixed(2)} fps` : 'H.264 · MP4 / MOV'}<span className="footer-shortcut">Space 재생 · ← → 이동 · ⌘ Z 실행 취소</span></span></footer>
    ...
    {clipRange && media && <PreviewRangeDialog clip duration={media.duration} start={clipRange.start} end={clipRange.end} onClose={() => setClipRange(null)} onPreview={range => { setClipRange(null); void run('export', range); }} />}
    ...
    {accentsOpen && media && <VisualAccents key={media.id} media={media} trackIndex={trackIndex} transcript={transcript} value={visualAccents} initialCue={accentCue} context={JSON.stringify([media.id, trackIndex, transcript, cuts, visualAccents, captionStyle])} captionStyle={captionStyle} disabled={!!busy} canUndo={accentsHistory.canUndo} canRedo={accentsHistory.canRedo} onUndo={() => { accentsHistory.undo(); setUnsaved(true); }} onRedo={() => { accentsHistory.redo(); setUnsaved(true); }} onChange={changeAccents} onEnableCaptions={() => changeCaptionStyle({ ...captionStyle, enabled: true })} onPreview={range => { setAccentsOpen(false); void run('preview', range); }} onClose={() => setAccentsOpen(false)} />}
    {captionsOpen && media && <CaptionEditor onEmphasize={id => { setCaptionsOpen(false); openAccents(id); }} glossary={glossary} onGlossaryChange={changeGlossary} media={media} trackIndex={trackIndex} transcript={transcript} captionStyle={captionStyle} onStyleChange={changeCaptionStyle} onRenderPreview={() => { setCaptionsOpen(false); void run('preview'); }} kept={kept} busy={!!busy} job={job} canUndo={captionHistory.canUndo} canRedo={captionHistory.canRedo} onChange={changeTranscript} onUndo={() => { captionHistory.undo(); setUnsaved(true); }} onRedo={() => { captionHistory.redo(); setUnsaved(true); }} onTranscribe={value => { void run('transcribe', undefined, value); }} onExport={() => { void run('captions'); }} onExportText={source => { void run('transcript', undefined, undefined, source ? 'source' : 'edited'); }} onCreateClip={range => { setCaptionsOpen(false); setClipRange(range); }} onCancel={cancel} onClose={() => setCaptionsOpen(false)} />}
    {effectsOpen && media && <EffectsEditor value={effects} contextId={`${media.id}:${trackIndex}`} cues={transcript?.trackIndex === trackIndex ? transcript.cues : []} duration={media.duration} time={time} kept={kept} connected={connectedEffects} onConnect={asset => setConnectedEffects(previous => new Set([...previous, asset.id]))} onImportBusy={setEffectImportBusy} canUndo={effectsHistory.canUndo} canRedo={effectsHistory.canRedo} onChange={changeEffects} onUndo={() => { effectsHistory.undo(); setUnsaved(true); }} onRedo={() => { effectsHistory.redo(); setUnsaved(true); }} onClose={() => setEffectsOpen(false)} onPreview={() => { setEffectsOpen(false); void run('preview'); }} />}
