...
export default function App({ cloud }: { cloud?: CloudEditor } = {}) {
  ...
  useEffect(() => {
    const onKey = (event: KeyboardEvent) => {
      ...
      if (dialog || aiOpen || captionsOpen || effectsOpen || restoreOpen || clipRange || previewRangeOpen || rangePreview || target.isContentEditable || ['INPUT', 'TEXTAREA', 'SELECT'].includes(target.tagName)) return;
  ...
  async function run(type: 'analyze' | 'preview' | 'export' | 'restore' | 'transcribe' | 'captions' | 'transcript', range?: { start: number; end: number }, transcription?: TranscriptionSettings, textMode?: 'source' | 'edited') {
    ...
    try {
      const cloudContext = cloud ? await cloud.save(makeProject(media, settings, trackIndex, cuts, speechProtection, transcript, captionStyle, effects, glossary), media) : {};
      if (active.cancelled) return;
      const started = await request<Job>('/jobs', { ...cloudContext, type, mediaId: media.id, settings, speechProtection, trackIndex, cuts, requestId: active.id, range, transcription, transcript, captionStyle, effects, textMode }, undefined, active.controller.signal);
