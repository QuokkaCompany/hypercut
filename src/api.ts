...
export async function request<T>(route: string, body?: unknown, method?: string): Promise<T> {
  const response = await fetch(`/api${route}`, { method: method || (body ? 'POST' : 'GET'), headers: { 'Content-Type': 'application/json', 'X-Hypercut-Token': token }, body: body ? JSON.stringify(body) : undefined });
...
export async function waitJob(id: string, update: (job: Job) => void): Promise<Job> {
  for (;;) { const job = await request<Job>(`/jobs/${id}`); update(job); if (job.status !== 'running') return job; await new Promise(resolve => setTimeout(resolve, 300)); }
}
