...
export function CaptionStyle({ value, media, text, language, disabled, onChange, onPreview }: { value: Style; media: Media; text: string; language?: string; disabled: boolean; onChange: (value: Style) => void; onPreview: () => void }) {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => { request<{ image: string; layout: { sizePercent: number } }>('/captions/style-preview', { mediaId: media.id, text, language, captionStyle: value }, undefined, controller.signal).then(result => { if (!controller.signal.aborted) { setPreview(result); setLoading(false); } }).catch(error => { if (!controller.signal.aborted) { setError(error.message); setLoading(false); } }); }, 180);
    return () => { clearTimeout(timer); controller.abort(); };
  }, [media.id, text, language, value]);
