...
The separate cloud beta handles authenticated accounts and uploaded media. Reports about tenant isolation, session/CSRF handling, chunk integrity, quota reservations, queue fencing and project revision conflicts are in scope. Cloud is currently a single-host, operator-managed beta; public anonymous registration and a hardened native-media sandbox are not implemented. Follow the [cloud operations guide](docs/cloud/operations.md), use HTTPS outside loopback, and keep FFmpeg, native inference dependencies and host/container software current.
