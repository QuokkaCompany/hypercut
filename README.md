...
VAD 모델(약 2.3MB)과 라이선스는 `assets/models/`에 포함됩니다. 추론 중 모델 다운로드나 AI 계정은 필요하지 않습니다. ONNX Runtime의 원격 진단은 로드 전에 끕니다. 프로젝트는 자막·디자인·효과음·교정 용어·끝 경계 검토 정보와 언어별 번역을 저장하는 v8로 생성합니다. 기존 v1은 보호 꺼짐·자막 없음, v2는 기존 보호 설정·자막 없음, v3는 기존 자막을 보존해 읽습니다. v1~v3를 열 때 MP4 자막 포함은 꺼져 있습니다. v4는 기존 스타일을 보존하며 v1~v4의 효과음은 빈 상태로 읽습니다. v1~v5의 교정 용어는 빈 상태로 읽습니다. v6의 기존 용어도 보존합니다. v7까지의 프로젝트를 읽을 수 있으며 이전 HyperCut 버전은 v8 프로젝트를 열지 못할 수 있습니다.

## 클립과 다국어 대본

- 메인 화면의 **클립 만들기**에서 원본 시작·끝 시각을 정합니다. 현재 컷·자막·효과음을 반영해 원본 해상도로 MP4를 만들고 **클립 MP4 저장**으로 저장합니다. 프레임 경계에 맞춘 실제 범위와 편집 후 길이를 결과에서 확인하세요.
- **전사와 자막**에서 시작 문장을 선택하고 **클립 마지막 문장**을 고른 뒤 **선택 문장으로 클립 만들기**를 누를 수도 있습니다. 연속된 원본 범위 안에서 현재 삭제한 구간은 제외합니다. 자막 중간을 자르면 범위를 넓히거나 MP4 자막 포함을 꺼 주세요.
- **전체 대본 TXT 저장**은 제외된 문장까지, **편집한 대본 TXT 저장**은 남은 문장만 UTF-8로 저장합니다. 편집본에서 일부가 잘린 문장은 문구·경계 확인 후 출력합니다.
- **번역할 언어**를 고르고 **현재부터 N개 AI 번역**을 누릅니다. 한 번에 최대 20개·4,000자를 기존 AI 연결 또는 채팅/MCP로 보내고 원문·제안을 비교해 선택 적용합니다. 나머지는 **다음 미번역으로 이동**으로 이어서 처리합니다.
- 번역은 원문·시각을 보존하고 언어별로 저장합니다. **출력 자막 언어**는 SRT·TXT·자막 포함 MP4에 공통 적용됩니다. 원본 청취 영상의 기본 자막은 원문입니다. 원문이 바뀌거나 번역이 빠지면 해당 출력이 막히며, 번역문을 직접 수정하거나 다시 번역할 수 있습니다.
- 출력 언어 변경과 번역 적용·수정은 실행 취소·다시 실행·프로젝트 저장에 포함됩니다. 재전사하면 새 대본으로 교체되므로 이전 번역이 새 문장에 자동 연결되지는 않습니다.

검증 범위와 재현 명령은 [클립·다국어 검증 기록](docs/testing/2026-09-06-multilingual-results.md)에 있습니다. 인증된 실제 AI 번역 품질과 사람 녹음 정확도 평가는 별도로 남아 있습니다.

## 로컬 전사와 자막

영상 선택 후 **자막**을 열어 언어·전사 채널을 고르고 **음성 전사 시작**을 누릅니다. 한국어·영어·일본어·중국어·스페인어·프랑스어·독일어·포르투갈어·이탈리아어·러시아어와 자동 감지를 선택할 수 있습니다. 한국어 기존 시험에 더해 영어·일본어·중국어 TTS의 실제 전사와 일본어 자동 감지를 확인했습니다. 중국어 TTS에서도 오인식이 있어 언어별 일반 녹음 정확도를 보장하지 않습니다. 앱 메뉴는 한국어입니다.
...
자막은 번들 Noto Sans KR 및 필요한 문자에만 로드하는 Noto Sans CJK KR 글꼴과 `@napi-rs/canvas`로 투명 이미지를 만들고 FFmpeg `overlay`로 합성합니다. FFmpeg의 `subtitles`/libass 필터는 필요하지 않습니다. 폰트 출처·해시는 [글꼴 명세](assets/fonts/manifest.json), SIL OFL 라이선스는 [OFL.txt](assets/fonts/OFL.txt), 실제 프레임·두 앱·오프라인 근거는 [자막 디자인 검증](docs/testing/2026-09-05-caption-rendering-results.md)에 있습니다. 사용자 컴퓨터의 다른 글꼴은 읽지 않습니다.
...
AI 메뉴에서 자연어로 무음 설정을 제안받고 현재 값과 비교해 적용합니다. 이 작업은 요청 문장과 네 가지 설정만 전달하며 적용 후 다시 분석해야 컷이 바뀝니다. 자막 창의 AI 교정은 선택한 문구·용어·교정 지시만 전달합니다. 효과음 제안은 선택한 음원의 별칭·설명, 클립·참고 자막의 원본 시각과 영상 유지 구간을 전달합니다. 번역은 선택 문구·용어·목표 언어·지시를 전달합니다. 모든 AI 작업은 원본 영상·음성·파일명을 자동 첨부하지 않습니다.
...
[자동화 탐색 비용 비교](docs/testing/2026-09-06-selector-overhead-results.md)의 CSS 조건 12회가 당시 측정 기준을 통과했습니다(최대 1.980GiB). 이후 전체 프레임 검사에서 컷 경계 결함을 발견했으므로, 이 결과는 수정 후 출력 정확성·성능의 근거가 아닙니다. 기존 역할 기반 메모리 실패도 유지합니다. [컷 경계 수정 검사](docs/testing/2026-09-06-composition-oracle-results.md)에서 모든 보존 프레임의 수·내용·시각과 60초 자막·효과음 합성을 확인했고, 수정된 두 앱의 60초 기본 편집 사전 검사도 통과했습니다. [두 앱 동시 합성](docs/testing/2026-09-06-composition-app-results.md)에서도 60초 자료의 컷·자막·효과음 출력과 취소·재시도를 확인했습니다. 첫 60분 동시 합성에서 메모리·자막 응답 기준을 초과해 디코더와 자막 목록을 개선했으며, 같은 조건을 재검증 중입니다.
