export function validateTranscript(value, duration) {
  ...
  const cues = value.cues.map(cue => {
    ...
    return { id: cue.id, start: cue.start, end: cue.end, text, ...(cue.reviewedFor ? { reviewedFor: cue.reviewedFor } : {}) };
...
export function mapCaptions(transcript, kept) {
  ...
  return transcript.cues.map(cue => {
    ...
    const reviewKey = JSON.stringify([cue.start, cue.end, cue.text, parts]);
    return { ...cue, outputStart: start, outputEnd: end, removed, needsReview: changed && cue.reviewedFor !== reviewKey, reviewKey };
  });
}
...
export function toSRT(transcript, kept) {
  ...
  if (mapped.some(cue => cue.needsReview)) throw new Error('컷과 겹치는 자막 문구를 먼저 검토해 주세요.');
