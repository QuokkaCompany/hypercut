...
assert.ok(['role', 'css'].includes(selector)); assert.ok(output.startsWith(path.resolve('test-output') + path.sep));
...
const report = { date: new Date().toISOString(), status: 'running', selector, scope: 'Read-only locator diagnostic: one real 60-minute analysis then a 1000-cue project. Three groups of 32 queries target the same enabled SRT button, without clicking it or editing. All app process trees are sampled during the queries. Driver excluded. No export, forced GC, browser restart, heap snapshot or performance gate claim. Backend heap/RSS comes through this test server parent IPC only.', sourceHashes: {}, snapshots: [], queryGroups: [], pageErrors: [] };
...
try {
  ...
  await page.waitForFunction(() => document.querySelector('.caption-source video')?.textTracks[0]?.cues?.length === 1000);
...
console.log(JSON.stringify({ status: report.status, selector, reportFile, peakGiB: report.resources?.peakBytes / 1024 ** 3, error: report.error }));
