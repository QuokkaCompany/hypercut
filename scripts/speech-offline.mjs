...
const child = spawn('/usr/bin/sandbox-exec', ['-p', profile, process.execPath, path.resolve('scripts/speech-e2e.mjs')], { stdio: 'inherit', env: process.env });
const [code, signal] = await once(child, 'exit'); assert.equal(code, 0, `Speech offline E2E exited ${code}/${signal}`);
const result = { status: 'PASS', externalConnectionProbe: denial, profile, scope: 'OS non-loopback network denial applied to actual browser, local server, bundled VAD inference and FFmpeg export. Generated Korean TTS only.', desktopOSIsolation: 'NOT_RUN: nested Electron sandbox limitation recorded in recovery report; native app security remains enabled.' };
await writeFile(path.resolve('test-output/speech-offline.json'), JSON.stringify(result, null, 2)); console.log(JSON.stringify(result));
