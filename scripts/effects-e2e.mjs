...
try {
  ...
  async function exercise(page, surface) {
    ...
    const stored = JSON.parse(await readFile(savedProject, 'utf8')); assert.equal(stored.version, 5); assert.equal(stored.effects.assets.length, 1); assert.equal(stored.effects.clips.length, 2); assert.equal(stored.effects.clips[0].start, 5); assert.equal(stored.effects.clips[0].offset, .1); assert.equal(stored.effects.clips[0].duration, .7); assert.equal(stored.effects.clips[0].gainDb, -7); assert.equal(stored.effects.clips[1].muted, true); assert.equal(stored.effects.assets[0].path, undefined);
    ...
    results.push({ surface, status: 'PASS', missingAssetBlocked: true, wrongReconnectRejected: true, sameHashReconnected: true, manualEditUndoRedo: true, muteDeleteUndo: true, dirtyDraftGuard: true, stalePreviewInvalidated: true, projectV5RoundTrip: true, captionAndEffectMP4Decoded: true, expectedBeepSeconds: [3, 3.7], rms, peak, quietPeak, sourcePreserved: true, nativeEffectOverwriteBlocked: surface === 'desktop' ? true : 'N/A', externalRequests: external.length, pageErrors: errors });
  }
