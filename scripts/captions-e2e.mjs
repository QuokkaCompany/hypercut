...
try {
  ...
  async function exercise(page, surface) {
    ...
    const project = JSON.parse(await readFile(projectPath, 'utf8')); assert.equal(project.version, 4); assert.equal(project.transcript.cues[0].text, corrected); assert.equal(project.transcript.cues[0].start, 1.1); assert.equal(project.transcript.cues.length, 2); assert.equal(project.cuts.length, 5);
