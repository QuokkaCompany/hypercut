...
try {
  ...
  async function exercise(page, surface) {
    ...
    const stored = JSON.parse(await readFile(savedProject, 'utf8')); assert.equal(stored.version, 7); assert.equal(stored.glossary, project.glossary); assert.equal(stored.transcript.cues[1].timingWarning.originalEnd, 16.24); assert.ok(stored.transcript.cues[1].reviewedFor); assert.equal(stored.transcript.cues[0].text, transcript.cues[0].text);
