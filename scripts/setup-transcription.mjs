...
async function download(url, file, hash, maxBytes) {
  let lastReport = 0;
  const result = await withDownloadSignals(signal => downloadVerifiedFile(url, file, hash, maxBytes, {
    signal,
    onProgress: ({ bytes }) => {
      if (Date.now() - lastReport > 2000) { console.log(`${path.basename(file)}: ${(bytes / 1024 ** 2).toFixed(1)} MiB`); lastReport = Date.now(); }
    }
  }));
  if (result.status === 'reused') console.log(`확인됨: ${path.basename(file)}`);
}
