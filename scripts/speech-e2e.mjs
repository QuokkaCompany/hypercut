...
try {
  ...
  async function exercise(page, surface) {
    ...
    assert.equal(project.version, 3); assert.deepEqual(project.speechProtection, { enabled: true, threshold: 0.5 });
    ...
    results.push({ surface, status: 'PASS', model: 'Actual bundled Silero VAD 6.2.1', input: 'Generated quiet Korean TTS; not user footage', baselineKeptSeconds: 0, speechProtectionKeptSeconds: 11.466667, cuts: 5, projectV3RoundTrip: true, projectV1Migration: true, previewAndSavedExport: true, externalBrowserRequests: external.length, pageErrors: errors });
  }
