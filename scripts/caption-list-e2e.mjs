...
for (const file of ['src/Captions.tsx', 'src/CaptionList.tsx', 'scripts/caption-list-e2e.mjs', 'dist/index.html', 'release/HyperCut-darwin-arm64/HyperCut.app/Contents/Resources/app.asar']) results.sourceHashes[file] = await sha256(file);
...
try {
  ...
  async function exercise(page, surface) {
    ...
    await button('전사와 자막').click(); assert.equal(await page.locator('.caption-row').count(), 1000);
    const text = page.locator('textarea[aria-label="자막 문구"]'), last = cues.at(-1);
    await button('자막 1000 선택').focus(); await button('자막 1000 선택').press('Enter');
    ...
    await text.fill('아직 적용하지 않은 문구'); page.once('dialog', dialog => void dialog.dismiss()); await button('자막 1 선택').click();
    ...
    page.once('dialog', dialog => void dialog.accept()); await button('자막 1 선택').click();
    await page.waitForFunction(expected => document.querySelector('textarea[aria-label="자막 문구"]')?.value === expected, cues[0].text);
    await button('자막 1000 선택').focus(); await button('자막 1000 선택').press('Space');
    await page.waitForFunction(expected => document.querySelector('textarea[aria-label="자막 문구"]')?.value === expected, last.text);
    ...
    await button('자막 1 선택').click(); await button('자막 1000 선택').click();
    ...
    await page.locator('input[type=file]').nth(1).setInputFiles(saved); await button('전사와 자막').click(); await button('자막 1000 선택').click();
    await page.waitForFunction(expected => document.querySelector('textarea[aria-label="자막 문구"]')?.value === expected, changed.text);
    assert.equal(Number(await page.locator('input[aria-label="자막 시작"]').inputValue()), changed.start);
    await button('자막 창 닫기').click(); assert.deepEqual(errors, []); assert.deepEqual(external, []);
    Object.assign(run, { status: 'PASS', allThousandRowsPresent: true, enterAndSpaceSelection: true, rejectedDraftDiscardPreserved: true, acceptedDraftDiscardApplied: true, editUndoRedo: true, currentTextAndSeek: true, fullProjectRoundTrip: true, pageErrors: errors, externalRequests: external.length });
  }
