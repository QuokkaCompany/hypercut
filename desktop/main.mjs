...
import { startGoServer as startServer } from './go-server.mjs';
...
ipcMain.handle('hypercut:save-project', async (event, data) => {
  ...
  const project = await server.control('validateProject', { project: data });
  ...
  await server.control('saveProject', { path: path.resolve(chosen.filePath), project });
  return true;
});
...
ipcMain.handle('hypercut:save-export', async (event, id) => {
  assertSender(event);
  const output = typeof id === 'string' && await server.control('export', { id });
  ...
  await server.control('saveExport', { id, path: target });
  return true;
});
