...
test('v7 preserves end-review state and v1-v6 keep existing cut review and glossary semantics', () => {
  ...
  assert.equal(project.version, 9); const restored = validateProject(JSON.parse(JSON.stringify(project)));
  assert.deepEqual(restored.transcript, transcript); assert.equal(mapCaptions(restored.transcript, kept).at(-1).needsReview, false);
  for (const version of [1, 2, 3, 4, 5, 6]) { const legacy = validateProject({ ...project, version }); assert.equal(legacy.version, 9); assert.equal(legacy.glossary, version < 6 ? '' : '용어'); }
