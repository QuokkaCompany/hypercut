...
export async function startServer(options = {}) {
  if (process.env.HYPERCUT_LOCAL_SERVER === "go" && (options.aiFetch || options.claudeCLI)) throw new Error("Use native Go AI tests for injected providers; JavaScript mocks cannot cross the Go process boundary.");
  if (process.env.HYPERCUT_LOCAL_SERVER === "go") return (await import("../../../desktop/go-server.mjs")).startGoServer(options);
