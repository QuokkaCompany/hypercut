...
최신 후보: 투명 자막 영역을 줄이는 커밋 `5002212`가 픽셀/미디어 회귀와 두 앱 60초 4회를 통과했다. 같은 입력·정답·탐색 러너로 `test-output/composition-strip-long/browser-long/`의 브라우저 60분 반복을 시작했다. 아직 진행 중이며 이전 반복 메모리 실패는 유지한다. [변경 및 사전 증거](../testing/2026-09-06-caption-strip-results.md).
