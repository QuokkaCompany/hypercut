...
작성일: 2026-09-06. 상태: 최신 긴 목록 후보 `3d572b4`의 두 앱 60초 합성 4회와 브라우저 60분 3회를 완료했다. 장시간 출력 정확성·조작·취소 후 재시도는 통과했지만 RSS 1.773 / 1.922 / 2.008GiB로 세 번째 메모리 실패다. 10분 및 Mac 장시간 조건은 확대하지 않았다. [최신 실행·감사 결과](../testing/2026-09-06-windowed-lists-results.md). [전사·자막·효과음 계획](2026-09-05-caption-effects-validation-plan.md)의 C05/C09/C10·FX05와 기존 긴 영상 성능 기준을 함께 확인한다.

## 입력과 독립 정답

이번에 검증한 후보는 [긴 목록의 화면 요소 제한](2026-09-06-windowed-lists-plan.md)이다. 전체 컷·자막 데이터와 실제 조작·출력 정답을 유지하고, 현재 표시 가능한 일부 행만 DOM에 둔다. 러너의 전체 개수 확인은 논리 목록 개수와 기존 전체 프로젝트/SRT 비교를 사용한다. 같은 입력·시간·RSS·취소 기준으로 사전 검사 후 재측정했다.
