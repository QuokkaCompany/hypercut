...
작성일: 2026-09-06. 상태: 실행 전. [전사·자막·효과음 계획](2026-09-05-caption-effects-validation-plan.md)의 C05/C09/C10·FX05와 기존 긴 영상 성능 기준을 함께 확인한다. 짧은 합성·개별 기능 통과를 긴 영상 동시 합성의 증거로 대체하지 않는다.
...
싱크·자막 매핑은 해당 위치 1프레임 이내, SRT는 1ms 반올림 이내여야 한다. 음소거/삭제 클립의 누출, 원본 변경, 저장 상태 손실, 자막/효과음이 빠진 거짓 성공은 0건이어야 한다. 음량 허용 오차는 PCM 정답과 AAC 부호화 차이를 짧은 정답 검출기 검증에서 먼저 고정하고 그 뒤 장시간 결과에 적용한다.
