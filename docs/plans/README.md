...
ChatGPT가 직접 편집 제안을 전달하는 경로는 [MCP 연결 계획](2026-09-06-chatgpt-mcp-plan.md)에서 별도로 정의했다. 공식 Secure MCP Tunnel 조건을 확인했으며, 로컬 프로토콜·앱 제안 수신·실제 계정 연결을 각각 검증한다. 현재 MCP 구현이나 계정 연결이 완료된 것은 아니다.
