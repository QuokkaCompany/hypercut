...
2026-09-06. [기본 검증 계획](2026-09-05-validation-plan.md)의 ‘시작이 차가운 실행과 캐시가 있는 실행을 구분한다’는 조건을 구체화한다. 현재 장시간 반복은 두 번째 실행의 메모리 기준을 초과했다. 이 계획은 다음 측정 방법을 준비하며 실패한 장시간 매트릭스를 확대하거나 기존 성능 게이트를 통과 처리하지 않는다.
...
2. 같은 내용의 새 시험 파일을 비캐시 순차 쓰기로 준비하는 도구를 추가한다. 이 부분은 아직 구현하지 않았다. APFS 복제·기존 파일 수정은 사용하지 않는다.
...
실제 앱에서의 입력 파일 캐시 없는 실행, 파일 복사 도구, OS 전체 cold-cache는 아직 수행하지 않았다. 실제 한국어 발화·CER·작업 시간 절감, Mac OS 네트워크 차단, 인증된 AI 연결도 별도 검증으로 남아 있다. 이번 사전 대조의 성공은 이 항목들의 통과 증거가 아니다.
