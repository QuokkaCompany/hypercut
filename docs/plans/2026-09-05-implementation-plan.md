...
1~5의 주요 동작과 6의 자연어 설정 제안 어댑터를 구현했다. [실행 결과](../testing/2026-09-05-preview-results.md)에 통과·부분 검증·미실행을 구분했다. 기존 ChatGPT·Claude 채팅은 우선 요청 복사/JSON 가져오기를 제공하며, 설계의 공식 구독 자동 연결을 대체 완료한 것으로 처리하지 않는다. 실제 발화 품질, 전체 장애 사례, 앱 전체 메모리, 인증된 AI 통합과 자막·효과음은 후속 작업으로 남아 있다.
