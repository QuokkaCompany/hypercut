...
작성일: 2026-09-05
상태: 계획 수립 완료. 제품 구현, 테스트 데이터 제작, 실험 실행 및 사용자 평가는 아직 수행하지 않았다.
대상 설계: [무음 자동 제거 MVP](../superpowers/specs/2026-09-05-hypercut-silence-first-design.md)
실행 방법: [테스트 계획](2026-09-05-test-plan.md)

## 1. 검증할 질문

첫 번째 출시 단위는 Mac에서 로컬 영상을 열어 음량 임계값으로 자동 컷하고, 필요 구간을 복원한 뒤 MP4로 내보내는 흐름이다. 아래 질문에 각각 증거가 있어야 완료로 판단한다.
...
- 후속 검증 대상: 로컬 LLM·API·구독 연동, VAD, 전사·자막 교정·디자인, 효과음. 이 기능들의 미실행 테스트를 MVP 통과 증거에 포함하지 않는다.
...
- 구현 프레임워크와 테스트 러너는 아직 결정되지 않았다. 이 계획의 행동·정답·증거 계약을 구현 도구 선택보다 우선한다.
...
기준 환경은 검증 시작 전에 모델명·칩·메모리·OS·저장 위치·전원 상태·미디어 도구 버전으로 고정한다. 현재 하드웨어 성능을 측정하지 않았으며 다음 수치는 초기 목표다.
...
- 시작이 차가운 실행과 캐시가 있는 실행을 구분한다. 미디어 작업은 조건별 3회 수행해 각 원시값과 중앙값·최댓값을 보고하며, 3회 결과로 p95를 주장하지 않는다.
...
| G2 MVP 기능 | 실제 앱의 파일 열기부터 내보내기, 편집·저장·취소·실패 | 모든 MVP P0/P1 테스트 통과, 지원 형식별 결과 확보 |
| G3 실사용 | 한국어 평가 영상, 품질·복원 부담·시간 절감·긴 영상 성능 | 위 품질·시간·성능 목표 충족. 충족하지 못한 조건을 지원 범위에서 명시적으로 처리 |
...
이 문서는 계획의 완료를 의미하며 제품 검증의 완료를 의미하지 않는다. 이후 구현과 실행 증거, 남은 미검증 항목은 [프리뷰 검증 결과](../testing/2026-09-05-preview-results.md)에 기록한다.
