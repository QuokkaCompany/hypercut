...
G1 핵심 미디어 검증 근거는 확보했다. G2에는 앱 전체 저장 중 강제 종료, 실제 OS의 기존 파일 교체 확인, 새 파일 교체 경합, Mac의 OS 네트워크 차단 등 남은 조건이 있다. G3의 실제 한국어 Q01~Q05, cold-cache·최종 빌드 성능도 미완료다. G4의 인증된 LLM 요청은 아직 수행하지 않았다. 로컬 VAD는 실제 모델 실행을 확인했지만 실제 녹음 품질·긴 영상 성능 평가가 남아 있다. 전사·자막·효과음과 ChatGPT MCP 통합도 후속 범위다.
