...
- [긴 영상 동시 합성 계획](../plans/2026-09-06-long-composition-plan.md): 10분/60분 컷·자막·효과음의 실제 출력·취소·저장·메모리·조작 검증. 독립 검증기와 두 앱 60초 동시 합성·취소·재시도 완료. 첫 60분의 메모리·자막 응답 실패 뒤 후보의 첫 재측정은 RSS 1.968GiB로 통과, 자막 p95 225ms로 실패했다. 자막 행 렌더링 후보의 두 앱 기능·60초 합성 4회를 완료했고 브라우저 60분 첫 실행은 RSS 1.962GiB·자막 p95 183ms로 통과했고 두 번째 실행 중이다.
...
| [두 앱 동시 합성](2026-09-06-composition-app-results.md) | 첫 60분의 메모리 2.205GiB·자막 p95 239ms 실패. 디코더 제한·자막 행 재사용 후보의 두 앱 60초 4회와 회귀 통과, 60분 첫 재측정 RSS 1.968GiB·자막 p95 225ms로 UI 실패. 행 렌더링 후보의 두 앱 기능·60초 합성 4회 통과 |
