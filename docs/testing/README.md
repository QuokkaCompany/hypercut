...
- [긴 목록 화면 요소 제한](2026-09-06-windowed-lists-results.md): 컷·자막 1,000개 데이터를 보존하며 보이는 항목과 선택/포커스를 유지한다. 두 앱 탐색·편집·저장 회귀 16개 실행/조건, 단위 80개와 패키징을 확인했다. 자막 견본의 CSS 잘림도 수정해 9개 표시 조건을 확인했다. 장시간 메모리는 재측정 전이다.
- [캐시 검증 준비](../plans/2026-09-06-cache-verification-plan.md): 자체 32MiB 시험 파일의 읽기 전 상주 페이지 0개·일반 읽기 뒤 2,048개를 확인했다. 실제 앱의 파일 캐시 없는 실행과 OS 전체 cold-cache는 미검증이다.
