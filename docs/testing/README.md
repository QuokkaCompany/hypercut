...
G1 핵심 미디어 검증 근거는 확보했다. G2에는 앱 전체 저장 중 강제 종료, 실제 OS의 기존 파일 교체 확인, 새 파일 교체 경합, Mac의 OS 네트워크 차단 등 남은 조건이 있다. G3의 실제 한국어 Q01~Q05, cold-cache·최종 빌드 성능도 미완료다. G4는 실제 인증된 모델 요청을 아직 수행하지 않았다. VAD·전사·자막·효과음과 ChatGPT MCP 통합도 후속 범위다.

현재는 음량 임계값 모드다. 음성 인식 또는 사람의 말 여부 판별 성공으로 표현하지 않는다. 작은 발화를 계속 자른다면 규칙 시험이 통과해도 제품 품질은 실패이며, 발화 보호 방식과 기본값을 다시 검토한다.
