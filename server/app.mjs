...
import { inspectMedia, publicMedia, analyzeMedia, exportMedia, playbackFile } from './media.mjs';
...
export async function createApp({ dataDir = path.join(projectRoot, '.hypercut'), distDir = path.join(projectRoot, 'dist'), development = false, aiFetch } = {}) {
  ...
  const registerFile = async (filePath, name) => { const item = await inspectMedia(filePath, name); media.set(item.id, item); return publicMedia(item); };
  let toolsStatus;
  ...
  app.post('/api/media', upload.single('video'), asyncRoute(async (req, res) => {
    ...
    try { res.json(await registerFile(req.file.path, Buffer.from(req.file.originalname, 'latin1').toString('utf8'))); }
  ...
  function startJob(type, item, settings, trackIndex, cuts) {
    if ([...jobs.values()].some(x => x.status === 'running')) throw new Error('진행 중인 작업을 완료하거나 취소한 뒤 다시 실행해 주세요.');
    if (jobs.size > 100) for (const [id, job] of jobs) { if (job.status !== 'running') jobs.delete(id); if (jobs.size <= 50) break; }
    ...
    job.task = Promise.resolve().then(async () => {
      const result = type === 'analyze' ? await analyzeMedia(item, settings, trackIndex, options) : await exportMedia(item, cuts, trackIndex, directory, options);
      if (controller.signal.aborted) { if (result.path) await rm(result.path, { force: true }); job.status = 'cancelled'; return; }
      if (type !== 'analyze') { exports.set(result.id, result); const { path: omitted, ...safe } = result; job.result = safe; }
  ...
  app.post('/api/jobs', asyncRoute(async (req, res) => {
    const { type, mediaId, settings, trackIndex, cuts } = req.body;
    if (!['analyze', 'export', 'preview'].includes(type)) throw new Error('지원하지 않는 작업입니다.');
    ...
    res.status(202).json(startJob(type, item, settings, trackIndex, cuts));
  }));
  ...
  app.delete('/api/jobs/:id', (req, res) => {
    const job = jobs.get(req.params.id);
    if (job?.status === 'running') { job.stage = '취소 중'; job.controller.abort(); }
    res.json({ cancelled: !!job });
  });
